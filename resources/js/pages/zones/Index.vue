...
import { CheckCircle2, CircleDollarSign, Eye, Pencil, Plus, Search, Trash2, Users, X } from '@lucide/vue';
...
<div class="space-y-6 p-4 md:p-6">
  ...
  <div class="rounded-2xl border bg-card shadow-sm">
    ...
    <tbody class="divide-y"><tr v-for="zone in props.zones.data" :key="zone.id" class="hover:bg-muted/20"><td class="px-5 py-4"><Link :href="`/zones/${zone.id}`" class="font-medium hover:text-primary">{{ zone.name }}</Link><div class="text-xs text-muted-foreground">Zone #{{ zone.id }}</div></td><td class="px-5 py-4"><span class="rounded-full bg-muted px-2.5 py-1 text-xs font-medium">{{ pricingLabel[zone.pricing_model] ?? zone.pricing_model }}</span></td><td class="px-5 py-4"><span :class="zone.is_online_bookable ? 'text-emerald-600' : 'text-muted-foreground'" class="text-xs font-medium">{{ zone.is_online_bookable ? 'Online bookable' : 'Not bookable online' }}</span></td><td class="px-5 py-4 text-muted-foreground">{{ zone.zone_spaces_count }} spaces</td><td class="px-5 py-4 text-muted-foreground">{{ formatDate(zone.created_at) }}</td><td class="px-5 py-4"><div class="flex justify-end gap-1"><Button variant="ghost" size="icon" title="View" as-child><Link :href="`/zones/${zone.id}`"><Eye class="size-4" /></Link></Button><Button variant="ghost" size="icon" title="Edit" @click="openEdit(zone)"><Pencil class="size-4" /></Button><Button variant="ghost" size="icon" title="Delete" :disabled="zone.zone_spaces_count > 0" @click="removeZone(zone)"><Trash2 class="size-4 text-destructive" /></Button></div></td></tr>
